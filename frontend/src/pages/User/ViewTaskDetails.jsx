...
const ViewTaskDetails = () => {
  ...
  // Convert frontend status to backend-compatible status
  ...
  const updateTodoChecklist = async (index) => {
    ...
    try {
      const response = await axiosInstance.put(
        ...
        { 
          todoChecklist,
          status: formatStatusForBackend(task.status) // send proper backend status
        }
      );
          ...
          <div className='grid grid-cols-1 md:grid-cols-4 mt-4'>
            <div className='form-card col-span-3'>
              <div className='flex items-center justify-between'>
                ...
                <div 
                  className={`text-[11px] font-medium ${getStatusTagColor(task?.status)} px-4 py-0.5 rounded`}
                >
                  {task?.status}
                </div>
              </div>
